...
against `MONO_ENV_API_URL`; the on-prem migration adds new desktop
surfaces the backend team must implement, plus moves the remaining NPS
...
| `MONO_ENV_BACKEND_URL`     | no          | falls back to `MONO_ENV_API_URL`                                                                      | Origin for NPS, token refresh, Gmail account linking, unsubscribe proxy, share, and push-WS. Set this if those routes live on a different host. |
...
— e.g. NPS — it pulls the token from main via IPC.)

---

## 2 · Email privacy proxy

### `POST ${MONO_ENV_BACKEND_URL}/desktop/unsubscribe`

The desktop client must not contact sender-controlled `List-Unsubscribe`
HTTPS URLs directly from the user's machine. When a user clicks unsubscribe,
Electron main validates that the URL is HTTPS, then asks the backend to make
...
{ "url": "https://sender.example/unsubscribe/opaque-token" }
```

Backend requirements:

- Require authentication and rate-limit per member/account.
- Accept only `https:` URLs, reject private/internal IP ranges after DNS
  resolution, and set short connect/read timeouts.
- Send the outbound unsubscribe request from backend egress, preferably with
  `POST` and `Content-Type: application/x-www-form-urlencoded`.
- Do not follow redirects to non-HTTPS, private, or internal hosts.
- Log only the destination origin and response status unless a short-lived
...
```json
{ "ok": false, "status": 400, "error": "Only HTTPS unsubscribe URLs are allowed" }
```

`mailto:` unsubscribe options cannot be proxied safely by the backend; the
desktop client opens those as explicit user mail actions only when no HTTPS
unsubscribe succeeded.

---

## 3 · Push — WebSocket replaces FCM
...
## 4 · Ported Cloud Functions endpoints
...
Valid `eventType` values: `feature_usage`, `support_interaction`,
`onboarding_complete`, `general_feedback`, `product_update`, `third_email`.
...
## 8 · Migration checklist (backend team)
...
- [ ] `POST /desktop/account-link-intents`, `/add-account`, and
      `POST /desktop/account-link-completions` for Gmail account linking.
- [ ] `POST /desktop/unsubscribe` backend privacy proxy for HTTPS
...
- [ ] `GET /nps/entries` and `POST /nps` ported from Cloud Functions to your
      backend origin (or a sibling origin if `MONO_ENV_BACKEND_URL` differs
      from `MONO_ENV_API_URL`).
- [ ] `/share/:id` route on `MONO_ENV_PUBLIC_DOMAIN`.
- [ ] Updater feed (S3/R2/nginx/whatever) at `MONO_ENV_UPDATE_FEED_URL`.

When all required surfaces are live, set the env vars in the client's build environment,
