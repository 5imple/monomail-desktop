...
interface ContactSuggestionInputProps extends MultipleSelectorProps {
  className?: string;
  contacts: Contact[];
...
const ContactSuggestionInput: React.FC<ContactSuggestionInputProps> = ({
  className,
  contacts,
  onSelectionChange,
  forbiddenInputs = [],
    ...
    return (
      <ContactProfileDropdown
        ...
        onOpenChange={(open) => {
          setActiveContacts((prev) => {
            if (open) {
              // Add the contact only if it's not already in the list
              if (!prev.find((o) => o.value === option.value)) {
                return [...prev, option];
              }
              return prev;
            } else {
              // Remove the contact when the context menu closes
...
export default ContactSuggestionInput;
