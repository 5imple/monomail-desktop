...
interface UnifiedInboxNavProps {}
const UnifiedInboxNav: FC<UnifiedInboxNavProps> = () => {
  ...
  const { getNavStateForSpace, toggleMailOpenForSpace, toggleItemExpandedForSpace } =
    useSidebarAtom();
  const { t } = useTranslation();
  const defaultNav = useDefaultNav();

  if (!activeSpace) {
    return null; // or return a loading state or fallback UI
  }

  // Get the current navigation state for this space
  const navState = getNavStateForSpace(activeSpace.id);
  const { isMailOpen, expandedItems } = navState;

  const accounts = authAccounts.filter((account) => activeSpace.accountUids.includes(account.uid));
  const hasMultipleAccounts = accounts.length > 1;

  // Toggle mail section open/closed
  const handleToggleMail = () => {
    toggleMailOpenForSpace(activeSpace.id);
  };

  // Toggle expanded state of a main item
  const handleToggleItemExpanded = (itemId: string) => {
    toggleItemExpandedForSpace(activeSpace.id, itemId);
  };
  ...
  return (
    <div id="mail-inbox" className="my-2 flex flex-col gap-3 px-2">
      <Collapsible open={isMailOpen} onOpenChange={handleToggleMail}>
                ...
                >
                  <MonoIcon
                    ...
                    className={cn(
                      ...
                      isMailOpen ? '' : '-rotate-90',
                      'text-muted-foreground'
                    )}
                  />
                </Button>
                <span className="text-sm text-muted-foreground">{t('sidebar.mail')}</span>
              </span>
            </div>
          </CollapsibleTrigger>
        </div>
