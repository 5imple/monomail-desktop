...
>(({ className, children, closeButton = true, ...props }, ref) => (
  <DialogPrimitive.Content
    ref={ref}
    className={cn(
      // `no-drag`: the app's full-width titlebar strip is `-webkit-app-region:
      // drag` at z-50, the same stacking level as this dialog. Without opting
      // out, the OS swallows clicks in the top ~52px as window-drags, so a
      // dialog's close (X) and any top-row controls become unclickable.
      'no-drag fixed left-[50%] top-[50%] z-50 grid w-full max-w-[720px] translate-x-[-50%] translate-y-[-50%] gap-5 rounded-lg border bg-background p-6 shadow-lg ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] md:mx-0',
      className
    )}
    {...props}
  >
