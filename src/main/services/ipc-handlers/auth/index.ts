...
import { microsoftOAuthServer } from '@/main/services/mangers/auth/MicrosoftOAuthServer';
import type { MicrosoftOAuthTokens } from '@/main/services/mangers/auth/MicrosoftOAuthServer';
...
export function registerAuthHandlers() {
  ...
  // ── Microsoft (work/school) direct OAuth ────────────────────────────────

  const microsoftMemberFromTokens = (tokens: MicrosoftOAuthTokens) => ({
    uid: tokens.uid,
    email: tokens.email,
    displayName: tokens.name,
    photoURL: tokens.picture
  ...
  ipcMain.handle(
    'main:auth:initiate-microsoft-sign-in',
    async (): Promise<{ ok: true } | { ok: false; error: string }> => {
      try {
        const tokens = await microsoftOAuthServer.startFlow();
        tokenManager.saveTokens({
          accessToken: tokens.accessToken,
          refreshToken: tokens.refreshToken,
          expiresInSec: tokens.expiresInSec,
          member: microsoftMemberFromTokens(tokens),
          provider: 'microsoft'
        });
        // Attach the identity metadata the refresh path can't recover later.
        tokenManager.saveMailAccountTokens({
          provider: 'microsoft',
          accessToken: tokens.accessToken,
          refreshToken: tokens.refreshToken,
          expiresInSec: tokens.expiresInSec,
        ...
        log.error('[auth:initiate-microsoft-sign-in]', (e as Error).message);
        return { ok: false, error: (e as Error).message };
      }
    }
  );
  ...
  ipcMain.handle(
    'main:auth:initiate-microsoft-add-account',
    async (): Promise<{ ok: true; accessToken: string } | { ok: false; error: string }> => {
      try {
        const tokens = await microsoftOAuthServer.startFlow();
        tokenManager.saveMailAccountTokens({
        ...
        log.error('[auth:initiate-microsoft-add-account]', (e as Error).message);
        return { ok: false, error: (e as Error).message };
      }
    }
  );

  // ── Provider-neutral account ops (renderer migrates onto these) ─────────
  ...
  ipcMain.handle(
    ...
    async (_, uid: string): Promise<{ ok: true } | { ok: false; error: string }> => {
      ...
      const removed = tokenManager.removeMailAccount(uid);
      if (!removed) {
        return { ok: false, error: 'Account not found in local token store' };
      }
      const mainAppWindow = windowManager.getMainAppWindow();
  ...
  ipcMain.handle(
    ...
    > => {
      try {
        const token = await tokenManager.getMailAccountAccessToken(uid);
        return { ok: true, ...token };
      } catch (e) {
        return { ok: false, error: (e as Error).message };
      }
